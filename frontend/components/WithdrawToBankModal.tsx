...
export default function WithdrawToBankModal({
  publicKey,
  onClose,
  ...
  const [transaction, setTransaction] =
    useState<AnchorTransactionRecord | null>(null);
    ...
    fetchAnchorEndpoints()
      .then(async (endpoints) => {
        ...
        if (endpoints.directPaymentServer) {
          ...
          if (isMounted && info) {
            setSupportsSep31(
              Object.values(info.receive || {}).some((cfg) => cfg.enabled),
            );
          }
        }
      })
  ...
  const startWithdraw = async () => {
    ...
    try {
      ...
      if (finalRecord?.status === "completed") {
        ...
        setPhase("error");
        setErrorMessage(
          `Withdrawal ended with status "${finalRecord.status}".`,
        );
      } else {
        setPhase("error");
        setErrorMessage(
          "Withdrawal timed out. Check the anchor's tracking page.",
        );
      }
    } catch (err: unknown) {
      setPhase("error");
    ...
    >
      ...
      {phase === "form" && (
        <div className="space-y-4">
          <label className="block">
            ...
            >
              {(availableAssets.length ? availableAssets : ["USDC", "XLM"]).map(
                (code) => (
          ...
          <p className="text-xs text-amber-700">
            You&apos;ll provide your bank details on the anchor&apos;s secure
            form. The anchor receives the {asset} payment from this wallet and
            pays out {fiat} to your registered account.
          </p>
      ...
      {phase === "loading" && (
        <p className="text-amber-200 text-sm">
          Authenticating with the anchor…
        </p>
      )}
            ...
            {transaction.external_transaction_id && (
              <div className="flex justify-between">
                ...
                <dd className="font-mono">
                  {transaction.external_transaction_id.slice(0, 14)}…
                </dd>
              </div>
            )}
          </dl>
