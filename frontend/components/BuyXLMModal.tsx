...
export default function BuyXLMModal({
  publicKey,
  onClose,
  onComplete,
}: BuyXLMModalProps) {
  ...
  const startDeposit = async () => {
    ...
    try {
      ...
      if (finalRecord?.status === "completed") {
        ...
        setPhase("error");
        setErrorMessage(
          "Deposit timed out. Check the anchor's tracking page for status.",
        );
      }
    } catch (error: unknown) {
      setPhase("error");
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-amber-700 mb-5">
        Powered by <span className="font-mono">{ANCHOR_HOME_DOMAIN}</span> via
        SEP-0024.
      </p>
      ...
      {phase === "idle" && (
        <div className="space-y-4">
          ...
          <p className="text-xs text-amber-700">
            You&apos;ll be redirected to the anchor&apos;s secure deposit page
            to enter your fiat payment details. The funds arrive in this wallet
            ({publicKey.slice(0, 6)}…{publicKey.slice(-4)}) once the anchor
            ...
            {transaction.stellar_transaction_id && (
              <div className="flex justify-between">
                ...
                <dd className="font-mono">
                  {transaction.stellar_transaction_id.slice(0, 10)}…
                </dd>
              </div>
            )}
          </dl>
