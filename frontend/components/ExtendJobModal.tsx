...
export default function ExtendJobModal({ job, onClose, onExtended }: Props) {
  ...
  return (
    ...
    >
      <h2
        id="extend-job-title"
        className="font-display text-xl font-bold text-amber-100 mb-2"
      >
        Extend Job Expiry
      </h2>
      <p className="text-amber-700/90 text-sm mb-4">
        Extend the duration of{" "}
        <span className="font-semibold text-amber-300">{job.title}</span>
      </p>
      ...
      <div className="flex gap-3">
        ...
        >
          {extending
            ? "Extending..."
            : `Extend (${formatXLM((option?.feeXlm || 0).toString())})`}
        </button>
      </div>
    </AccessibleModal>
  );
