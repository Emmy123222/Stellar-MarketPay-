...
pub enum DataKey {
    ...
    MessageCid(String),
    /// Issue #440 — admin-settable ceiling on referrer bonus payouts.
    /// Stored as a raw i128 (token stroops, same units as escrow amounts).
    /// Absent => no cap (2% bonus applies as before, backward compat).
    MaxReferrerBonusXlm,
}
