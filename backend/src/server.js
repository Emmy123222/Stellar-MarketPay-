...
// Realtime subscribers are indexed by logical room instead of receiving every
// event emitted by the API. Room names are intentionally opaque to the server:
// clients may subscribe to `job:<id>` and `user:<address>`.
  ...
  if (url.pathname === "/ws/realtime") {
    ...
    if (token) {
      try {
        ...
        // Anonymous realtime subscriptions are still allowed.
      }
    }
