...
// Expose the shared scope client registry + JSON sender so the REST scope
// routes (src/routes/scope.js) can lock a session and broadcast the change to
// every WebSocket collaborator without owning the WebSocket server.
app.locals.scopeSessionClients = scopeSessionClients;
    ...
    ws.on("message", async (raw) => {
      try {
        ...
        if (message.type === "scope:update") {
          // Each connection keeps its own session snapshot, so re-read the
          // persisted row: a finalize (over WebSocket or REST) must lock the
          // document for every collaborator, not just the one who signed off.
          const freshSession = await loadScopeSession(sessionId);
          if (!freshSession) {
            sendJson(ws, "scope:error", { error: "Session not found or expired" });
            return;
          }
          if (freshSession.finalized) {
            sendJson(ws, "scope:error", { error: "Session is finalized and locked" });
            return;
          }
