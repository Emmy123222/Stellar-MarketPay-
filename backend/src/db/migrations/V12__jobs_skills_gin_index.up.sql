...
-- Back-fill from the job_skills junction table. The junction table is only
-- created in V14, which normally runs *after* this file because of duplicated
-- version numbers across migrations — so skip the back-fill when it does not
-- exist yet (fresh databases have an empty skills column here regardless).
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'job_skills') THEN
    UPDATE jobs j
    SET skills = (
